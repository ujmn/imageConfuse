...
ImageObfuscater::ImageObfuscater()
{

}

void ImageObfuscater::swap_pixel( uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2 ) {
...
void ImageObfuscater::swap_cell( rect a, rect b ) {
        uint32_t ax = a.x;
        uint32_t ay = a.y;
        uint32_t bx = b.x;
        uint32_t by = b.y;
...
ImageObfuscater::ImageObfuscater( uint32_t * data, uint32_t img_width, uint32_t img_height, uint32_t cell_size )
        : main_cells()
        , bottom_sides()
        , right_sides()
        , corner()
...
void ImageObfuscater::obfuscate() {
